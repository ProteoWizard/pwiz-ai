...
1. Read [CRITICAL-RULES.md](CRITICAL-RULES.md) - Absolute constraints (limit: 100 lines)
2. Read [MEMORY.md](MEMORY.md) - Project context and critical gotchas (limit: 200 lines)
3. Read [WORKFLOW.md](WORKFLOW.md) - Git workflows and TODO system (limit: 200 lines)
4. Read active TODO in [todos/active/](todos/active/) - Current branch context

**Budget: 500 lines** for essential context - that is the sum of the three limits
above, not a measurement. For where these files actually stand right now:

```powershell
pwsh -File ./ai/scripts/audit-docs.ps1 -Section ai
```

**Need more?** See [TOC.md](TOC.md) for a complete index of every document with
descriptions and current line counts, regenerated by `ai/scripts/Generate-TOC.ps1`.

## Core Files (Read Every Session)

Each core file carries a line limit (100-200) so it stays quick to load. The limits
are declared in [docs/documentation-maintenance.md](docs/documentation-maintenance.md);
counts are never hand-recorded here, because a hand-maintained count rots and a rotted
...
- **Line-limited** (100-200 each, <1000 combined) - Quick to load and scan
