<#
.SYNOPSIS
    Osprey same-impl regression test against a frozen snapshot.
    Asserts that the current Osprey build produces the same
...
.DESCRIPTION
    ...
    known-good earlier Osprey build instead of a parallel run
    of Rust osprey. Used as the safety net for the Osprey
    ...
    1. Default (compare):
       ...
       Runs the current Osprey build at every stage and compares
       against $TestBaseDir/<dataset>/_snapshots/<tag>/<stage>/. Fails
       on any mismatch.

    2. -CreateSnapshot:
         pwsh -File Test-Snapshot.ps1 -CreateSnapshot
       Runs the current Osprey build at every stage and copies
    ...
    Stage isolation matrix (matches Test-Regression.ps1):
      ...
      (Osprey CLI; Rust osprey still uses the old --no-join/--join-at-pass flags)
...
.PARAMETER Tool
    ...
    Osprey.exe at pwiz_tools/Osprey/...) or Rust (osprey.exe
...
param(
    ...
    # Same-impl snapshot side: CSharp (Osprey.exe) or Rust
...
switch ($Tool) {
    ...
    'Rust' {
        ...
        $toolBuildHint   = 'pwsh -File ./ai/scripts/Osprey/Build-OspreyRust.ps1   (or: cd C:\proj\osprey && cargo build --release)'
    }
}
...
function Run-Stage1to4 {
    ...
    # Osprey uses --task PerFileScoring for the Stage 1-4 worker;
