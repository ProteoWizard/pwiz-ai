<#
.SYNOPSIS
    Production-mode performance measurement for Osprey (C#) vs osprey (Rust).
...
$tools = @{
    ...
    'Rust' = @{
        ...
        BuildHint = 'pwsh -File ./ai/scripts/Osprey/Build-OspreyRust.ps1'
    }
}
...
# stage7. C# Osprey emits `[STAGE-WALL] second-pass-fdr` for the
...
function Invoke-PipelineRun {
    ...
    # C# Osprey gates [STAGE-WALL]/[TIMING] behind --perf-stats (off by default);
    ...
    # parallel, Astral (hram, ~3 GB spectra/file) sequential. Osprey's
    ...
    # are present (always emitted by Osprey in --no-bundle production
