<#
.SYNOPSIS
    Osprey full regression: drives Test-Snapshot across both
    datasets and reports a single pass/fail summary.

.DESCRIPTION
    Wraps Test-Snapshot.ps1 to make the canonical Osprey-alone
    ...
    Modes (mutually exclusive switches):
      ...
      -CreateSnapshot
                 ...
                 by reviewers.  Records the current Osprey
...
.EXAMPLE
    ...
    pwsh -File ./ai/scripts/Osprey/Test-Full-Regression.ps1

.EXAMPLE
    # Refresh the frozen baseline after an approved behavior change
    pwsh -File ./ai/scripts/Osprey/Test-Full-Regression.ps1 -CreateSnapshot
#>
