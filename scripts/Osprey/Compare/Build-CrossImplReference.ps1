...
.SYNOPSIS
    ...
    cross-impl gate (Compare-CrossImpl-Reference.ps1) compares Osprey
...
.DESCRIPTION
    ...
    Note the deliberate Osprey divergence (commit #4261): Osprey does
    ...
    Osprey's Stage-4 scores.parquet to THIS set's .scores.stage4.parquet
...
$readme = @"
# Osprey cross-impl reference set -- $Dataset

Frozen Rust ``osprey`` outputs that the occasional cross-impl gate
(``Compare-CrossImpl-Reference.ps1``) compares Osprey against, so the gate
...
- Osprey Stage-4 ``scores.parquet`` -> this set's ``.scores.stage4.parquet`` (parquet 1e-9). Like stage to like stage: Osprey does NOT overwrite scores.parquet at Stage 6 (commit #4261); Rust does.
...
- ``output.blib`` -> SQL row+col 1e-9 (``osprey_version`` whitelisted -- Osprey is on the Skyline version scheme intentionally).
