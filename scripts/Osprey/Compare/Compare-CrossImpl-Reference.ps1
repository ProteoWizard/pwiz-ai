<#
.SYNOPSIS
    Occasional cross-impl gate: run Osprey straight-through and compare its
...
.DESCRIPTION
    ...
    occasional double-check that Osprey still matches Rust osprey at every
    ...
    Stage-correct mapping: Osprey's scores.parquet is the STAGE-4 output
...
if ($SkipCs -and (Test-Path (Join-Path $csDir 'output.blib'))) {
    ...
    Write-Host "[C#] Osprey straight-through ($Framework) ..." -ForegroundColor Cyan
    Push-Location $csDir
    try {
...
foreach ($stem in $stems) {
    # Stage 4: Osprey scores.parquet (Stage-4) vs reference .scores.stage4.parquet
...
if ($nFail -eq 0) {
    Write-Host "OVERALL: PASS -- Osprey matches the frozen Rust reference at every gated boundary." -ForegroundColor Green
    exit 0
} else {
