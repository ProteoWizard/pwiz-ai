<#
.SYNOPSIS
    Side-by-side lines-of-code audit: Osprey (C#) vs osprey (Rust fork).
...
.DESCRIPTION
    ...
    C# modules (pwiz_tools/Osprey/Osprey.*) pair with Rust
    crates (osprey/crates/osprey-*) by name:

        Osprey.Core          <-> osprey-core
        Osprey.IO            <-> osprey-io
        Osprey.Chromatography <-> osprey-chromatography
        Osprey.Scoring       <-> osprey-scoring
        Osprey.FDR           <-> osprey-fdr
        Osprey.ML            <-> osprey-ml
        Osprey               <-> osprey (main exe)
        Osprey.Test          <-> inline #[cfg(test)] in each crate
...
.PARAMETER CSharpRoot
    Path to the Osprey solution directory.
    Default: <projRoot>\pwiz\pwiz_tools\Osprey,
...
.EXAMPLE
    pwsh -File ./ai/scripts/Osprey/Audit-Loc.ps1 -CSharpRoot D:/other/Osprey
...
# Resolve project root from this script's location: ai/scripts/Osprey -> ai/scripts -> ai -> projRoot
$projRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $PSScriptRoot))
if (-not $CSharpRoot) { $CSharpRoot = Join-Path $projRoot 'pwiz\pwiz_tools\Osprey' }
...
$modules = @(
    @{ Name = 'Core';            CS = 'Osprey.Core';           Rust = 'crates\osprey-core' },
    @{ Name = 'IO';              CS = 'Osprey.IO';             Rust = 'crates\osprey-io' },
    @{ Name = 'Chromatography';  CS = 'Osprey.Chromatography'; Rust = 'crates\osprey-chromatography' },
    @{ Name = 'Scoring';         CS = 'Osprey.Scoring';        Rust = 'crates\osprey-scoring' },
    @{ Name = 'FDR';             CS = 'Osprey.FDR';            Rust = 'crates\osprey-fdr' },
    @{ Name = 'ML';              CS = 'Osprey.ML';             Rust = 'crates\osprey-ml' },
    @{ Name = 'Main (entry)';    CS = 'Osprey';                Rust = 'crates\osprey' }
)

Write-Host 'Counting Osprey (C#) and osprey (Rust) source lines...' -ForegroundColor Cyan
...
foreach ($m in $modules) {
    # C# project folder (note: Osprey main is a nested folder, same name as module)
...
# Tests: Osprey.Test is a separate project; Rust tests are inline
# in each crate's src (cannot cleanly split).
$testCs = Invoke-Cloc -Path (Join-Path $CSharpRoot 'Osprey.Test') -Language 'C#'
...
Write-Host 'Osprey (C#) vs osprey (Rust)  -  lines of source code per matching module' -ForegroundColor Cyan
Write-Host $sepLine -ForegroundColor Cyan
Write-Host ''
...
Write-Host ''
Write-Host ('Osprey code (production):     {0,8}  ({1})' -f (Format-Number $csTotalCode), (Format-Kloc $csTotalCode)) -ForegroundColor Green
Write-Host ('Osprey code + Osprey.Test: {0,6}  ({1})' -f (Format-Number $csPlusTests), (Format-Kloc $csPlusTests)) -ForegroundColor Green
...
Write-Host ('  Osprey.Test (separate project): {0} files, {1} code lines ({2} total)' -f `
...
$null = $sb.AppendLine('|---|---:|---:|---:|')
$null = $sb.AppendLine(('| Osprey production (7 library + main projects) | {0} | {1} | {2} |' -f `
    (Format-Number $csTotalFiles), (Format-Number $csTotalCode), (Format-Kloc $csTotalCode)))
$null = $sb.AppendLine(('| Osprey.Test (separate project) | {0} | {1} | {2} |' -f `
    (Format-Number $testCs.Files), (Format-Number $testCs.Code), (Format-Kloc $testCs.Code)))
$null = $sb.AppendLine(('| **Osprey production + tests** | **{0}** | **{1}** | **{2}** |' -f `
...
$null = $sb.AppendLine('`Osprey.Test` project, so we show whether that project''s total aligns')
...
$null = $sb.AppendLine('  `--include-lang` set to `C#` for Osprey and `Rust` for osprey.')
...
$null = $sb.AppendLine('- Regenerate with: `pwsh -File ./ai/scripts/Osprey/Audit-Loc.ps1`')
