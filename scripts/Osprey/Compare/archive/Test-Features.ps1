<#
.SYNOPSIS
    Feature parity + Stages 1-4 perf test: Osprey vs Rust Osprey
...
.PARAMETER CsharpRoot
    Path to the pwiz checkout whose Osprey binary should be used. When
    omitted, auto-detects: tries pwiz-work1, pwiz, pwiz-work2 in order and
    picks the first one with a built Osprey.exe for -TargetFramework.
...
# C# binary: auto-detect which pwiz worktree has a built Osprey.exe for
# the requested -TargetFramework. Override with -CsharpRoot.
$csharpRelBin = "pwiz_tools\Osprey\Osprey\bin\x64\Release\$TargetFramework\Osprey.exe"
if ($CsharpRoot) {
...
foreach ($bin in @($rustBinary, $csharpBinary)) {
    if (-not (Test-Path $bin)) {
        ...
        Write-Host "Build first with Build-Osprey.ps1 / Build-OspreyRust.ps1" -ForegroundColor Yellow
        exit 1
    }
}
...
try {
    ...
    Write-Host "Step 2: Running Osprey (C#) $modeLabel..." -ForegroundColor Cyan
