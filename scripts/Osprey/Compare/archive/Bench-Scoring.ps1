...
.SYNOPSIS
    ...
    memory. Compares upstream Rust, fork Rust, and Osprey C# on Stellar
    or Astral data.
...
.PARAMETER SkipCSharp
    Skip the Osprey (C#) run. Useful when comparing two Rust binaries
...
if (-not $SkipCSharp) {
    $results += Run-Benchmark "Osprey (C#)" $csharpBin $true "CSharp"
}
