...
.DESCRIPTION
    Runs both Rust osprey and Osprey in --join-at-pass=1 mode against all
...
.PARAMETER CsharpRoot
    pwiz worktree with built Osprey.exe (auto-detect if omitted).
...
if (-not (Test-Path $csharpBinary)) { Write-Error "Osprey binary not found: $csharpBinary"; exit 1 }
