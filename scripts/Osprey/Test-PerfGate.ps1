<#
.SYNOPSIS
    Osprey performance gate: same-session A/B of a branch build vs the
...
.DESCRIPTION
    ...
    Why a dedicated A/B and not a number-in-HTML compare: Osprey perf is
...
.PARAMETER WarmupRuns
    ...
    until Osprey settles -- discarding them keeps the measured reps
...
.PARAMETER BranchRoot
    ...
    'pwiz' next to ai/ (same rule as Build-Osprey.ps1).
...
.EXAMPLE
    ...
    pwsh -File ./ai/scripts/Osprey/Test-PerfGate.ps1

.EXAMPLE
    # Heavier check before a perf-sensitive merge.
    pwsh -File ./ai/scripts/Osprey/Test-PerfGate.ps1 -Dataset Both

.EXAMPLE
    # Re-judge from already-built binaries (no rebuild).
    pwsh -File ./ai/scripts/Osprey/Test-PerfGate.ps1 -SkipBuild
...
# Resolve the branch worktree (sibling 'pwiz' by default, same rule as Build-Osprey.ps1).
if (-not $BranchRoot) {
...
foreach ($key in $variants.Keys) {
    $argsSrc = Join-Path $variants[$key].Root 'pwiz_tools\Osprey\Osprey\OspreyCommandArgs.cs'
    $hasFlag = $false
...
function Invoke-PerfRun {
    ...
    if ($exit -ne 0) { throw "${VariantKey}/${DatasetName}/run${RunIdx}: Osprey exited $exit (see $logPath)" }
