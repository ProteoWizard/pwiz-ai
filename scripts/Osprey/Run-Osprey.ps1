<#
.SYNOPSIS
    Run Osprey or Rust Osprey on test data (Stellar or Astral)

.DESCRIPTION
    Runs either the C# (Osprey) or Rust (Osprey) implementation on a
...
.PARAMETER ExitAfterScoring
    ...
    Osprey, or --no-join for Rust osprey. Matches what
...
try {
    ...
    # --task PerFileScoring for Osprey, --no-join for Rust osprey
