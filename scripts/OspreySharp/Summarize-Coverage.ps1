<#
.SYNOPSIS
    Summarize an OspreySharp dotCover JSON coverage report.

.DESCRIPTION
    Reads the JSON report produced by `Build-OspreySharp.ps1 -Coverage`
    (dotCover) and prints a whole-project picture aimed at a test-coverage
    review:
      - overall statement coverage across the OspreySharp.* assemblies
    ...
    selects a single project), this walks every OspreySharp.* assembly in the
...
$assemblies = $coverage.Children | Where-Object {
    ($_.Kind -eq "Assembly" -or $_.Kind -eq "Project") -and $_.Name -like "OspreySharp*"
}

if (-not $assemblies) {
    Write-Host "No OspreySharp* assemblies found in the coverage report." -ForegroundColor Red
...
Write-Host "OSPREYSHARP TEST COVERAGE SUMMARY" -ForegroundColor Cyan
Write-Host $separator -ForegroundColor Cyan
Write-Host ""
