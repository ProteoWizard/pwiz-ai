<#
.SYNOPSIS
    Symmetric cross-impl smoke test for OspreySharp Bug B port: the C#
...
.DESCRIPTION
    ...
    Here we exercise the C# WRITE side:
      ...
      Phase 2 (first-join, --join-at-pass=1 --join-only):      C#    OspreySharp.exe   <-- writes v2 envelope
...
if (-not (Test-Path $ospreyShExe)) { throw "OspreySharp.exe not found at $ospreyShExe" }
