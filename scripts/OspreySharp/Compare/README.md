...
gate OspreySharp output against Rust osprey (`maccoss/osprey` or a
local fork).  Now that OspreySharp is the primary implementation,
these scripts are used rarely -- the OspreySharp-alone regression
gate (`../Test-Snapshot.ps1` / `../Test-Full-Regression.ps1`) handles
the day-to-day correctness checks.  Keep this folder when you need
to ask:

  "Did my OspreySharp change drift us from Rust at the 1e-9 gate?"
...
```powershell
pwsh -File ./ai/scripts/OspreySharp/Compare/Build-OspreyRust.ps1
```
