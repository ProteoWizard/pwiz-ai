...
# Script location: ai/scripts/AutoQC/
# Target: pwiz_tools/Skyline/Executables/AutoQC/
...
$autoQCRoot = Join-Path $pwizRoot 'pwiz_tools/Skyline/Executables/AutoQC'
...
try {
    ...
    # Path: ai/scripts/Skyline/scripts/Sync-DotSettings.ps1
    $syncScript = Join-Path $aiRoot 'scripts/Skyline/scripts/Sync-DotSettings.ps1'
...
$buildCmd = "& `"$msbuildPath`" AutoQC.sln -restore /p:Configuration=$Configuration /p:Platform=`"$Platform`" /nologo /verbosity:$Verbosity"
...
if ($RunTests) {
    ...
    if (-not (Test-Path $testDll)) {
        $declared = Select-String -Path "AutoQCTest\AutoQCTest.csproj" -Pattern '<TargetFrameworks?>([^<]+)<' |
            Select-Object -First 1
        if ($declared) {
            $tfm = ($declared.Matches[0].Groups[1].Value -split ';' | Where-Object { $_.Trim() } | Select-Object -Last 1).Trim()
            if ($tfm) { $testDll = "AutoQCTest\bin\$Configuration\$tfm\AutoQCTest.dll" }
        }
    }
    if (-not (Test-Path $testDll)) {
