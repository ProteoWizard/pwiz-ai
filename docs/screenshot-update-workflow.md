...
Each tutorial has a `cover.png` that is a composite image used on the Skyline start page and tutorial landing pages. Cover shots are generated by the test framework during auto-screenshot mode.

### How Cover Shots Work

1. **`IsCoverShotMode`** — Tests check this property to know when cover shot generation is active.

2. **Collecting images during the test** — Some tests capture intermediate images for later compositing:
   ```csharp
   if (IsCoverShotMode)
   {
...
3. **`TakeCoverShot()`** — Called at the end of the test to capture the main Skyline window. This calls `ProcessCoverShot()` to post-process the image, then saves it as `cover.png`.

4. **`ProcessCoverShot(Bitmap bmp)`** — Virtual method in `AbstractFunctionalTest`. The base implementation calls `bmp.CleanupBorder()`. Tests override this to composite additional images onto the cover.

### ProcessCoverShot Override Pattern

Tests that need to overlay additional images (search logs, Excel screenshots, figures) override `ProcessCoverShot`. **Critical**: On Windows 11, `CleanupBorder()` returns a **new** Bitmap (unlike Win10 which modifies in place). Overrides must capture and return the result of `base.ProcessCoverShot()`:

```csharp
// CORRECT — works on both Win10 and Win11
protected override Bitmap ProcessCoverShot(Bitmap bmp)
{
    ...
    graph.DrawImageUnscaled(_overlayImage, result.Width - _overlayImage.Width - 10,
        result.Height - _overlayImage.Height - 30);
    return result;
}

// WRONG — only works on Win10 where base returns the same reference
protected override Bitmap ProcessCoverShot(Bitmap bmp)
{
    using var graph = Graphics.FromImage(base.ProcessCoverShot(bmp));
    graph.DrawImageUnscaled(_overlayImage, bmp.Width - _overlayImage.Width - 10, ...);
    return bmp;  // BUG: returns original, not the processed bitmap!
}
```

### Tests with ProcessCoverShot Overrides

| Test File | Overlay Content |
|---|---|
| `TestPerf/DdaTutorialTest.cs` | DDA search log image |
| `TestPerf/DiaUmpireTutorialTest.cs` | DIA-Umpire search log image |
| `TestPerf/DiaSearchTutorialTest.cs` | DIA search log image |
| `TestPerf/AcquisitionComparisonTutorialTest.cs` | Figure 1 image from tutorial folder |
| `TestTutorial/ExistingExperimentsTutorialTest.cs` | Excel spreadsheet image |

### Key Files

- `TestUtil/TestFunctional.cs` — `ProcessCoverShot()` base (line ~2067), `TakeCoverShot()` (line ~2073)
- `TestUtil/ScreenshotManager.cs` — `TakeShot()` which invokes `processShot` delegate
