...
(`ProteoWizard_OspreyWindowsNetPerfRegressionTests`) runs
...
`astral` (3, hram). The Stellar mzML is searched three times, against two library
files - the third configuration is the second library with its decoy rows stripped
at setup (`StripDecoys`), not a third library:

| Dataset | mzML | Library | Decoys | `--model-diagnostics` |
|---|---|---|---|---|
...
entrapment true-FDP oracle exists to measure it. The library-decoy leg never calls
the generator; the entrapment-free legs have nothing to measure against. Its
`MaxPass1Fdp = 0.02` ceiling is committed and deliberately NOT regenerated by
`-CreateGolden` - the pre-fix b<->y decoy construction measured 11.81% there.
