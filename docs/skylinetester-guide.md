...
The **Run type** combo chooses what the night does (each type is one job, so no machine does
everything in one night); it maps to these TestRunner arguments on top of the common
`offscreen=on quality=on loop=-1 runsmallmoleculeversions=on retrydatadownloads=on`:

| Run type | TestRunner arguments | What happens |
|---|---|---|
...
it from its own branch/type setting (see `pwiz_tools/Skyline/SkylineNightly/RunSpec.cs`).
Locally, `Run-Tests.ps1 -PerfFirst` reproduces the perf order and `-Quality -Pass 1` the leak
pass on a chosen test list.
