...
Use **`ai/.tmp/`** for all temporary and working files — never `ai/tmp/` or other locations.

This is the shared workspace for session artifacts that are never committed to Git:
- **MCP tool output**: nightly test reports, exception reports, wiki page content
- **Diagnostic files**: profiling results, screenshot diffs, test failure logs
- **Cross-session handoffs**: strategy docs, research notes, plans from Claude.ai or other tools
- **Communication files**: any file generated for a human or another AI session to pick up

Many MCP tools and skills automatically write output here. When you need to save
...
- **skyline-development** → Any work involving Skyline: code changes, tests, builds, MCP tools, planning, GitHub issues, or codebase questions
