...
- [ ] Preserve pinned rulers across annotation-display toggles in `GraphSpectrum` — toggling a spectrum context-menu option (Show m/z, Show observed m/z, Show mass error, etc.) currently rebuilds the graph via `ClearGraphPane()`, which unconditionally clears `_pinnedSeriesKeys`. Pinned rulers should survive these redraws and only reset on actual precursor change (mirror the `_lastPrecursorId` pattern already used in `GraphFullScan` at line 1845; apply to `GraphSpectrum` and `ViewLibraryDlg` as well if applicable).
- [ ] (Deferred from Copilot review #10) Extract a shared menu-builder helper for `GraphSpectrum`, `GraphFullScan`, and `ViewLibraryDlg` so the ruler context-menu logic and `_pinnedSeriesKeys` plumbing live in one place. Larger refactor; deferred from the current PR to keep correctness fixes focused.
