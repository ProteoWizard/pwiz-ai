...
### 2026-08-12 - GOLDENS REBASELINED (`54796e5d5e`), validated by arithmetic

`regression.ps1 -Dataset All -CreateGolden` PASSED. 21 files moved across all four datasets.
Committed separately from code, as #4553 did.

**This branch now MOVES THE DISCOVERY SET** - it did not before, and the earlier claim that
`mode1 (vs golden)` passes on all four is superseded. A target no longer inherits its paired
decoy's experiment q, so pair-losers drop out of the reported set: Stellar precursors
29,364 -> 29,300, confirmed IDENTICALLY by both implementations, so blib content and the
derived tables move with it.

**Validated by arithmetic, not by an additive diff.** Pass-1 experiment decoys against
`(targets + entrapment) x 1%`:

| dataset | targets + entrapment | expected | golden `decoy.n` | ratio |
|---|---|---|---|---|
| astral | 110,249 | 1,102 | **1,101** | 1.00 |
| stellar-libdecoy | 26,784 + 128 | 269 | **268** | 1.00 |
| stellar-gendecoy-entrap | 28,694 + 228 | 289 | **288** | 1.00 |

astral previously carried **542,368** in that cell - 4.6x more decoys than targets - and passed
a +N/-0 additive check. Additive and garbage are not exclusive.

`MaxPass1Fdp` and `MaxAbsTilt` are deliberately NOT regenerated by `-CreateGolden`, so those
Tier-2 bounds survive the rebaseline as an independent check.

**PUSH STATE**: pwiz `54796e5d5e` and maccoss/osprey `02d3df0` are both PUSHED and consistent.
The C# lookup bug was C#-only (Rust was correct), so no Rust change accompanies it.

**STILL RUNNING at handoff**: `regression.ps1 -Dataset All` against the NEW goldens, log at
`ai/.tmp/verify-all-after-rebaseline.log`. Check it before trusting the rebaseline.
