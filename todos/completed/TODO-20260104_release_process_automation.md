...
**On master (PR #3757):**
- ~~`pwiz_tools/Skyline/Jamfile.jam` - Version update (SKYLINE_YEAR 25→26)~~ **REVERTED**
...
### 2026-01-05 (continued) - PR Creation and Docker Verification
...
- Tag confirmed: `skyline_daily_26.0.9.004-7b2495b` at DockerHub
- Pulled and tested image locally using Docker Desktop (Linux container mode)
- Verified SkylineCmd works correctly:
  ```
  Skyline-daily (64-bit : automated build) 26.0.9.004 (7b2495b)
      ProteoWizard MSData 3.0.26004
  ```

**Docker Testing Command (for future reference):**
```bash
# On Windows with Git Bash, use MSYS_NO_PATHCONV to prevent path mangling
...
- Git Bash mangles paths starting with `/` - use `MSYS_NO_PATHCONV=1` prefix
- The `wine64_anyuser` wrapper has sudo issues; direct `wine` invocation works
- SkylineCmd location: `/wineprefix64/drive_c/pwiz/skyline/SkylineCmd.exe`

### 2026-01-05 (continued) - Version Constraint Discovery

**Problem**: PR #3757 is failing tests on TeamCity:
- `TestDocumentFormatCurrent`: Expected 26.1, got DocumentFormat.CURRENT 25.11
- `TestMostRecentReleaseFormatIsSupportedForSharing`: Missing 26.1 in SupportedForSharing

**Root cause**: We incorrectly updated `SKYLINE_YEAR` from 25 to 26 on master during FEATURE COMPLETE.
The version cannot be updated to 26.x until the MAJOR release because:
1. `DocumentFormat.CURRENT` must match the version (25.11 ≠ 26.1)
2. Format updates require creating `Skyline_26.1.xsd` schema file
3. `SupportedForSharing()` must include the new version

**Key insight**: These tests skip for Build=9 (FEATURE COMPLETE) but run for Build=1 (daily).
So the release branch (26.0.9) passes, but master (26.1.1) fails.

**Solution**: Revert the Jamfile.jam version change in PR #3757. The two remaining commits
(cherry-pick workflow + UpgradeDlg fix) are still valid and should be kept.

**Updated workflow understanding**:
- Master is **release-frozen** during FEATURE COMPLETE (no daily releases published)
- Master is **development-open** (PRs merge freely, new features accumulate)
...
### ~~Commit 1: Update master to 26.1.1 versioning~~ **REVERTED**

~~**File:** `pwiz_tools/Skyline/Jamfile.jam`~~

**This commit was reverted** - cannot update version to 26.x until MAJOR release.
See "Version Constraint Discovery" section above for details.

The version update will happen during the MAJOR release workflow when:
- `DocumentFormat.CURRENT` is updated to 26.1
- `Skyline_26.1.xsd` schema is created
- `SupportedForSharing()` includes 26.1

### Commit 2 → 1: Update cherry-pick workflow for new release branch
...
### Commit 3 → 2: Fix UpgradeDlg version display for FEATURE COMPLETE releases
