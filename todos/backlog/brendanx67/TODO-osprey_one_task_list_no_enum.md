# One authoritative task list: `--task` looks up an `OspreyTask`, the `HpcTask` enum and its switches go

**Status**: Backlog. Raised by Brendan 2026-09-19 reviewing [#4686](https://github.com/ProteoWizard/pwiz/pull/4686).
**Module**: `osprey`
**Base**: `Skyline/work/20260612_net8_port` - all Osprey development is on the port branch. Start it
AFTER #4686 merges (that PR makes each task's `TASK_NAME` the one spelling of its name; this one
makes the task instances the one list). Worktree `C:\proj\pwiz-work2`.
